...
import {
  ...
  Link2, Clock, Loader2, ShieldCheck, User,
} from "lucide-react";
              ...
              <div className="px-5 py-4 space-y-3 overflow-y-auto" style={{ maxHeight: "55vh" }}>
                ...
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  <div className="flex items-center gap-2">
                    <Link2 size={14} style={{ color: "#475569" }} className="flex-shrink-0"/>
                    <p className="text-sm font-semibold break-all" style={{ color: "#e2e8f0" }}>{log.linkName}</p>
                  </div>
                  {log.linkSlug && (
                    <p className="text-xs mt-1 ml-5" style={{ color: "#475569" }}>/{log.linkSlug}</p>
                  )}
                </div>
...
/* ════════════════════════════════════════
   MAIN
════════════════════════════════════════ */
export default function AdminLinkLogs() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      const r = await fetch(`${API}/api/links/logs?${params}`, {
  ...
  const handleDeleteTrigger = (log) => {
    setDeleteId(log._id);
    setDeleteName(log.linkName);
    setDeleteOpen(true);
  };

  const confirmDelete = async () => {
    try {
      await fetch(`${API}/api/links/logs/${deleteId}`, {
  ...
  const confirmClearAll = async () => {
    try {
      await fetch(`${API}/api/links/logs`, {
        method: "DELETE", headers: { Authorization: `Bearer ${getToken()}` },
      });
    ...
    <>
      ...
      <div className="ll-wrap max-w-5xl mx-auto pb-10">
        ...
        <ConfirmModal isOpen={clearOpen} onClose={() => setClearOpen(false)} onConfirm={confirmClearAll} danger
          ...
          message={t("সব লিংক লগ স্থায়ীভাবে মুছে যাবে।","This will permanently delete all link logs.")}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs mt-1" style={{ color: "#475569" }}>
                    {t("এডমিন ও সাবএডমিনের লিংক কার্যক্রম ট্র্যাক করুন।","Track all link activity by admin and subadmin.")}
                  </p>
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input type="text"
              placeholder={t("লিংক বা ব্যবহারকারী খুঁজুন...","Search link or user...")}
        ...
        <div className="rounded-2xl overflow-hidden"
          ...
          <div className="ll-col-header">
            ...
            <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "#2d3f55" }}>{t("লিংক","Link")}</div>
                  ...
                  <motion.div key={log._id} className="ll-row"
                    ...
                    <div className="min-w-0 pr-2">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <Link2 size={11} className="flex-shrink-0" style={{ color: "#334155" }}/>
                        <span className="truncate text-sm font-medium" style={{ color: "#e2e8f0" }}>
                          {log.linkName}
                        </span>
                      </div>
                      {log.linkSlug && (
                        <p className="hidden sm:block text-[10px] mt-0.5 ml-4" style={{ color: "#334155" }}>/{log.linkSlug}</p>
                      )}
