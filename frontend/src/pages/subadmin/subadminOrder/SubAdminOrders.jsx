          ...
          <motion.div
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 20px", flexShrink:0, borderBottom:"1px solid rgba(99,102,241,0.08)", background:"rgba(99,102,241,0.02)" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:11, fontWeight:700, padding:"5px 12px", borderRadius:99, background:currentCfg.bg, color:currentCfg.textColor, border:`1.5px solid ${currentCfg.border}`, display:"flex", alignItems:"center", gap:5, boxShadow:`0 2px 8px ${currentCfg.dotColor}22` }}>
                  <span style={{ width:6, height:6, borderRadius:"50%", background:currentCfg.dotColor, display:"inline-block", boxShadow:`0 0 0 3px ${currentCfg.dotColor}28` }} />{currentCfg.label}
                </span>
                ...
                {isFinished ? (
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    ...
                    <motion.button onClick={handleSave} disabled={saving||!finalStatus}
                      ...
                      style={{ width:"100%", padding:"12px 20px", borderRadius:13, fontSize:13.5, fontWeight:700, color:"#fff", border:"none", cursor:!finalStatus?"not-allowed":"pointer", opacity:!finalStatus?0.45:1, background:hasCancelled?"linear-gradient(135deg,#f43f5e,#e11d48)":"linear-gradient(135deg,#6366f1,#4f46e5)", boxShadow:!finalStatus?"none":hasCancelled?"0 6px 20px rgba(244,63,94,0.3)":"0 6px 20px rgba(99,102,241,0.3)", display:"flex", alignItems:"center", justifyContent:"center", gap:8, transition:"all .15s", letterSpacing:"-.01em" }}>
        ...
        {isOpen && (
          <motion.div
            ...
            <motion.div
              ...
              onClick={e => e.stopPropagation()}>
              <h3 style={{ fontSize:16, fontWeight:900, color:"#0f172a", fontFamily:"'Plus Jakarta Sans',sans-serif", letterSpacing:"-0.025em" }}>{title}</h3>
              <p style={{ fontSize:13, marginTop:8, color:"#64748b", lineHeight:1.6 }}>{message}</p>
              <div style={{ display:"flex", gap:12, marginTop:24 }}>
                <button onClick={onClose} style={{ flex:1, padding:"12px 0", borderRadius:12, fontSize:13, fontWeight:600, background:"rgba(99,102,241,0.06)", border:"1px solid rgba(99,102,241,0.13)", color:"#475569", cursor:"pointer" }}>{cancelText}</button>
                <button onClick={onConfirm} style={{ flex:1, padding:"12px 0", borderRadius:12, fontSize:13, fontWeight:700, color:"#fff", cursor:"pointer", border:"none", background:"linear-gradient(135deg,#f43f5e,#e11d48)", boxShadow:"0 6px 20px rgba(244,63,94,0.32)" }}>{confirmText}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
